...
Protect the topics you have not finished watching, reading or playing. This Manifest V3 extension conceals matching text and related thumbnails locally, with optional reveal/preview controls (off by default) and no account requirement.
...
Solid mask is the default; Gaussian blur, motion blur and a pixelated mask are also available. Reveal controls and hover reveal are off by default. YouTube protection and previews have separate switches. Detection handles ordinary HTTP/HTTPS documents, new posts, text edits, lazy image labels and frames; it cannot recognize image pixels, video/audio, or shadow-root content. It is heuristic and may miss spoilers or hide innocent text. Initial content can appear before detection finishes.
...
Watchlists and preferences stay in local extension storage. There is no browsing telemetry, remote model, shared proxy, Google sign-in or external account-token bridge. Optional first-party accounts support explicit settings upload/download and administrator-configured keyword quotas. Optional TMDB lookup requests permission when enabled and uses your own credential directly against TMDB. Search labels omit plot summaries. Backups exclude credentials. See [privacy and permissions](extension/privacy.html).

An update under the same extension ID migrates supported old sync settings. Loading a different unpacked extension ID cannot access the old installation's storage; use a settings export/import instead. The release does not transfer Google accounts, learning weights or watched-title history. Free allows 2 literal custom keyword entries, Premium 10, and Max has no tier quota; existing device safety limits still apply. Title packs and aliases do not count. See [release notes and checks](docs/RELEASE_NOTES.md).

## Website and administrator panel

Run `npm run dev`, then open `http://127.0.0.1:4173`. This creates private local admin credentials in ignored `server/data/local-admin.json` and a connected development extension in `dist/local-extension/`. The promotional site is in `website/`; the working account/admin API is in `server/`. For public deployment and a configured release build, follow [accounts and deployment](docs/ACCOUNTS_AND_DEPLOYMENT.md).
...
- [Live-streaming proposal and suggested pricing](docs/STREAMING_AND_PRICING.md)
