...
chrome.runtime.onInstalled.addListener((details) => {
  ...
  // CRITICAL: Open onboarding on fresh install
  try {
    if (details && details.reason === 'install') {
      // Check if onboarding has been completed
      chrome.storage.sync.get(['onboardingComplete'], (store) => {
        if (!store.onboardingComplete) {
          // Open onboarding flow for first-time users
    ...
    console.debug('Could not open onboarding/welcome page:', e);
  }
});
