 ...
 */
function extractTrackedTitles(store) {
  const titles = [];
...
/**
 * Create V4-only processor (NO PHRASE MATCHING)
 */
function createV4Processor(trackedTitles, store) {
  console.log('[V4.5] Creating V4-only processor for titles:', trackedTitles);
  
  return {
    walk: function(node) {
      if (window.SpoilerShieldV4 && window.SpoilerShieldV4.isActive()) {
        walkWithV4Only(node, trackedTitles, store);
      } else {
...
function walkWithV4Only(node, trackedTitles, store) {
  if (!node) return;
  
  if (node.nodeType === Node.TEXT_NODE) {
    processTextWithV4(node, trackedTitles, store);
    return;
  }
  ...
  if (node.nodeType === Node.ELEMENT_NODE) {
    if (store.settings?.blurImages && /IMG|PICTURE|FIGURE|VIDEO/.test(node.nodeName)) {
      processImageWithV4(node, trackedTitles, store);
    }
  }
  
  const children = node.childNodes;
  for (let i = 0; i < children.length; i++) {
    walkWithV4Only(children[i], trackedTitles, store);
  }
}

function processTextWithV4(node, trackedTitles, store) {
  ...
  try {
    ...
    if (result.blurEntireBlock) {
      blurEntireBlock(parent, result, store);
    } else {
      blurText(node, text, result, store);
    }
  } catch (error) {
...
function processImageWithV4(el, trackedTitles, store) {
...
function blurEntireBlock(startNode, result, store) {
  ...
  while (blockElement && depth < 5) {
    const tag = blockElement.nodeName;
    if (['P', 'DIV', 'ARTICLE', 'SECTION', 'BLOCKQUOTE'].includes(tag)) {
      break;
    }
...
function observeMutations(processor) {
  ...
  const processBatch = () => {
    ...
    try {
      const batchToProcess = mutationBatch.splice(0, 50);
      ...
      Array.from(nodesToProcess).slice(0, 25).forEach(n => {
  ...
  const observer = new MutationObserver(mutations => {
    if (mutationBatch.length > 200) {
      mutationBatch = mutationBatch.slice(-50);
    }
...
async function reprocessNow() {
  ...
  try {
    ...
    const trackedTitles = extractTrackedTitles(store);
    ...
    console.log('[V4.5] Processing with V4 intelligence for titles:', trackedTitles);

    installStyles(store);
    
    const processor = createV4Processor(trackedTitles, store);
