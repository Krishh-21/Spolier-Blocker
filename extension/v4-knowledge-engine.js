...
class KnowledgeEngine {
  ...
  getKnowledge(trackedItems) {
    if (!trackedItems || !Array.isArray(trackedItems)) return [];
    return trackedItems.map(item => this.buildUniversalKnowledge(item));
  }

  /**
   * Build universal knowledge for ANY movie/show/game (sync — uses stored knowledge only)
   */
  buildUniversalKnowledge(item) {
    ...
    if (media) {
      ...
      media = { ...media, aliases: [...(media.aliases || [])],
        characters: (media.characters || media.entities || []).map(c => ({ ...c, aliases: [...(c.aliases || [])] })) };
    } else {
      media = this.createUniversalTemplate(title, mediaType, tmdbId);
    }

    // Add user-provided character phrases
    if (phrases && phrases.length > 0) {
      this.addCharacterPhrases(media, phrases);
    }

    // Add stored knowledge if available
    if (storedKnowledge) {
      this.mergeStoredKnowledge(media, storedKnowledge);
    }

    // Ensure universal patterns are always available
    media.universalPatterns = this.getUniversalPatterns();
    media.isUniversal = !this.knowledgeBase.has(key);
  ...
  mergeStoredKnowledge(media, knowledge) {
    ...
    if (knowledge.teams?.length) {
      media.teams = [...new Set([...(media.teams || []), ...knowledge.teams])];
    }
    if (knowledge.categories?.length) {
      media.categories = [...new Set([...(media.categories || []), ...knowledge.categories])];
    }
    if (knowledge.locations?.length) {
      media.locations = [...new Set([...(media.locations || []), ...knowledge.locations])];
    }
    if (knowledge.tagline) media.tagline = knowledge.tagline;
    if (knowledge.creators?.length) media.creators = knowledge.creators;
    if (knowledge.wikiPageId) media.wikiPageId = knowledge.wikiPageId;
    if (knowledge.wikidataId) media.wikidataId = knowledge.wikidataId;
  }

  /**
   * Create universal knowledge template for any title
   */
  createUniversalTemplate(title, mediaType, tmdbId) {
    return {
      title: title,
      type: mediaType,
      tmdbId: tmdbId,
      ...
      keywords: [],
      genres: [],
      universalPatterns: this.getUniversalPatterns(),
      isUniversal: true
    };
  }

  /**
   * Get universal spoiler patterns that work for ANY media
   */
  getUniversalPatterns() {
    return [
      // Death patterns (work for any character)
  ...
  /**
   * Add character phrases to media knowledge
   */
  ...
  /**
   * Fetch data from TMDB API (placeholder for future implementation)
   */
  async fetchTMDBData(tmdbId) {
    // TODO: Implement actual TMDB API call
    // For now, return null - this is a placeholder for future enhancement
    return null;
  }

  /**
   * Merge TMDB API data into media knowledge
   */
  mergeTMDBData(media, tmdbData) {
    if (tmdbData.cast) {
      for (const actor of tmdbData.cast) {
        if (actor.character && actor.name) {
          media.characters.push({
            name: actor.character,
            aliases: [actor.character, actor.name],
            fromTMDB: true
          });
        }
      }
  ...
  /**
   * Check if phrase is likely a character name
   */
  isValidCharacterPhrase(phrase, title) {
    const p = String(phrase || '').trim();
    if (!p || p.length < 2) return false;
    if (p.toLowerCase() === String(title || '').toLowerCase()) return false;
    if (/^[a-z0-9_.-]+$/.test(p) && !p.includes(' ')) return false; // URLs, IDs
    if (/^\d{4}$/.test(p)) return false; // Years
    if (p.length > 50) return false; // Too long
    return true;
  }
  ...
  checkRelevance(text, trackedKnowledge) {
    ...
    for (const media of trackedKnowledge) {
      if (this.textIncludesTerm(lowerText, media.title)) {
      ...
      if (media.type === 'awards') {
        ...
        if (media.categories) {
          for (const cat of media.categories) {
            if (this.textIncludesTerm(lowerText, cat)) {
      ...
      // Keywords/genres/categories — stored for matching, not standalone relevance
    }
  ...
  textIncludesTerm(text, term) {
    ...
    if (/[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}]/u.test(t)) return text.includes(t);
...
class EntityMatcher {
  ...
  matchEntityName(lowerText, entity) {
    ...
    if (primary.length >= 4 && this.knowledge.textIncludesTerm(lowerText, primary)) {
      return primary;
    }
    if (primary.includes(' ')) {
      ...
      if (first.length >= 4 && this.knowledge.textIncludesTerm(lowerText, first)) return first;
      if (last.length >= 4 && last !== first && this.knowledge.textIncludesTerm(lowerText, last)) return last;
    }
    for (const alias of (entity.aliases || [])) {
      const n = String(alias || '').trim();
      if (!n || n.length < 3) continue;
      if (this.knowledge.textIncludesTerm(lowerText, n)) return alias;
    }
    return null;
  }
}
...
class UniversalSpoilerEngine {
  ...
  detect(text, relevance, foundEntities) {
    ...
    const strongRelevance = relevance.some(r =>
      ['title_match', 'alias_match', 'entity_match', 'entity_alias_match', 'team_match'].includes(r.reason)
    );
    const strongPlotVerb = /\b(dies?|died|killed|kills?|murdered|dead|reveals?|revealed|sacrifices?|betrays?|betrayed|twist|wins? the|won the|defeats?|champion|finale|sacrifice|becomes? emperor|takes? the throne|stays? in the past|retires?)\b/i.test(text);
    ...
    if (hasNarrativeMoment && (entityCount >= 1 || highSpecificity)) {
      disclosures.push({
    ...
    if (entityCount >= 1 && strongPlotVerb && text.length >= 20) {
      disclosures.push({
        type: 'plot_advancement',
        severity: 'major',
        confidence: 0.86,
        reason: 'Character linked to plot outcome',
        universal: true
      });
    }

    if (entityCount >= 1 && hasPlotVerb && !strongPlotVerb && text.length >= 25) {
      // Weak verbs (discovers, becomes) need TMDB+Wiki corroboration: 2+ entities or full name
      const fullNames = foundEntities.filter(e => (e.matchedAs || '').includes(' ')).length;
      if (fullNames >= 1 && foundEntities.length >= 2) {
        disclosures.push({
          type: 'plot_advancement',
          severity: 'major',
          confidence: 0.82,
          reason: 'Multi-character plot discussion (corroborated)',
          universal: true
        });
      }
    }
