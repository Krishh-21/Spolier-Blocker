...
async function completeOnboarding() {
  if (selectedTitles.length > 0) {
    // Save selected titles to storage
    const store = await new Promise(resolve => 
      chrome.storage.sync.get(['selectedMedia'], resolve)
    );
    
    const existing = store.selectedMedia || [];
    const newTitles = selectedTitles.map(title => ({
      title: title.title,
      phrases: [title.title.toLowerCase()]
    }));
    
    // Merge with existing (avoid duplicates)
    const merged = [...existing];
    newTitles.forEach(newTitle => {
      if (!merged.find(t => t.title.toLowerCase() === newTitle.title.toLowerCase())) {
