...
async function searchMedia(query, mediaType) {
  ...
  // Request runtime host permission for the chosen host (proxy or TMDB)
  let originHost = '';
  try {
    if (TMDB_PROXY_URL) {
      ...
      originHost = 'https://api.themoviedb.org/*';
    }
  } catch {
    setStatus('Invalid TMDB proxy URL', true);
    return;
  }
  ...
  if (originHost) {
    ...
    if (!hasPerm) {
      ...
      if (!granted) {
        setStatus('Permission required to perform searches. Grant permission in the dialog.', true);
        return;
      }
    }
  }
  ...
  let url = '';
  if (TMDB_PROXY_URL) {
    url = `${TMDB_PROXY_URL.replace(/\/$/, '')}/3/search/${mediaType}?query=${encodeURIComponent(query)}`;
  } else {
    url = `https://api.themoviedb.org/3/search/${mediaType}?api_key=${encodeURIComponent(TMDB_API_KEY)}&query=${encodeURIComponent(query)}`;
  }
  try {
    ...
    if (!response.ok) {
      setStatus(`Search failed: ${response.status}`, true);
      return;
    }
