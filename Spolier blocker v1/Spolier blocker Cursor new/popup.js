...
async function searchMedia(query, mediaType) {
  ...
  const store = await getStore();
  const wikiEnabled = store.settings?.wikipediaEnabled !== false;
  const wikiTypes = ['sports', 'reality', 'awards', 'event', 'game'];

  if (wikiTypes.includes(mediaType)) {
    if (!wikiEnabled) {
      setStatus('Enable Wikipedia in Options to search sports/reality/events.', true);
      return;
    }
    setStatus('Searching Wikipedia…');
    const results = await WikipediaKnowledgeBuilder.search(query, mediaType, {
      contact: store.wikipediaContact || ''
    });
    if (!results.length) {
      setStatus('No Wikipedia results.', true);
      return;
    }
...
async function addTitle(item) {
  ...
  setStatus('Building knowledge profile…', false);

  const profile = await buildTitleProfile(item);
  const year = (item.release_date || item.first_air_date || '').split('-')[0] || '';
  ...
  const mediaType = profile.mediaType || item.media_type || item.type || 'movie';
  const entry = {
    title,
    phrases,
    type: mediaType,
    tmdbId: profile.tmdbId || item.id || null,
    wikiPageId: profile.wikiPageId || item.wikiPageId || null,
  ...
  const saveAndFinish = (list, statusMsg) => {
    chrome.storage.sync.set({ selectedMedia: list }, () => {
      if (chrome.runtime.lastError) {
        console.warn('Storage quota:', chrome.runtime.lastError.message);
        const slim = { title, phrases, type: mediaType, tmdbId: entry.tmdbId };
        const slimList = (selected || []).filter(s => String(s.title || '').toLowerCase() !== lower);
        slimList.push(slim);
        chrome.storage.sync.set({ selectedMedia: slimList }, () => {
          setStatus('Title added (compact — storage limit)', false);
          updateSelectedList();
          triggerReprocess();
        });
        return;
  ...
  const sources = profile.knowledge?.sources || [];
  const charCount = profile.knowledge?.characters?.length || 0;
  const srcLabel = sources.includes('wikipedia') && sources.includes('tmdb') ? 'TMDB+Wiki'
    : sources.includes('wikipedia') ? 'Wikipedia' : 'TMDB';
  const statusMsg = charCount
    ? `Added "${title}" (${charCount} names via ${srcLabel})`
    : `Added "${title}"`;
  saveAndFinish(next, statusMsg);
}
...
function getStore() {
  return new Promise(resolve => chrome.storage.sync.get({
    settings: { enabled: true, wikipediaEnabled: true },
    customKeywords: [],
    tmdbKey: '',
    tmdbProxy: '',
    wikipediaContact: ''
...
async function buildTitleProfile(item) {
  try {
    const store = await getStore();
    const aggressiveness = (store.settings && typeof store.settings.aggressiveness === 'number')
      ? store.settings.aggressiveness : 2;
    const wikiEnabled = store.settings?.wikipediaEnabled !== false;
    const wikiContact = store.wikipediaContact || '';
    const mediaType = item.media_type || item.type || (item.name && !item.title ? 'tv' : 'movie');
    const title = item.title || item.name || '';
    const profiles = [];
    const wikiPrimary = typeof WikipediaKnowledgeBuilder !== 'undefined'
      && WikipediaKnowledgeBuilder.isWikiMediaType(mediaType);
    const useTmdb = !wikiPrimary && ['movie', 'tv'].includes(mediaType) && item.id && !item.wikiPageId;

    if (useTmdb) {
      await ensureApiKey();
      profiles.push(await TmdbKnowledgeBuilder.buildForItem(item, {
        apiKey: TMDB_API_KEY,
    ...
    if (wikiEnabled && title && (wikiPrimary || item.wikiPageId || !useTmdb)) {
      const wikiProfile = await WikipediaKnowledgeBuilder.buildForTitle(title, mediaType, {
        contact: wikiContact,
        aggressiveness
      });
      profiles.push(wikiProfile);
    } else if (wikiEnabled && useTmdb && title) {
      const tmdbProfile = profiles[0];
      const thin = !tmdbProfile?.knowledge?.characters?.length;
      if (thin) {
        profiles.push(await WikipediaKnowledgeBuilder.buildForTitle(title, mediaType, {
          contact: wikiContact,
          aggressiveness
        }));
      }
    ...
    for (let i = 1; i < profiles.length; i++) {
      merged = WikipediaKnowledgeBuilder.mergeProfiles(merged, profiles[i]);
    }
    if (useTmdb && merged.knowledge && !merged.knowledge.sources.includes('tmdb')) {
      merged.knowledge.sources = ['tmdb', ...(merged.knowledge.sources || [])];
    }
    merged.mediaType = mediaType;
    return merged;
  } catch (e) {
    console.warn('buildTitleProfile failed', e);
    const title = item.title || item.name || '';
    return {
      phrases: [title.toLowerCase()],
      knowledge: null,
...
/** @deprecated Use buildTitleProfile — kept for any external callers */
async function expandPhrases(item) {
  const profile = await buildTitleProfile(item);
  return profile.phrases.filter(p => p !== (item.title || item.name || '').toLowerCase());
}
