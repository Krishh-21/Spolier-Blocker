...
function isAiChatSite() {
  ...
  return /gemini\.google|chat\.openai|chatgpt\.com|claude\.ai|copilot\.microsoft|perplexity\.ai|poe\.com|you\.com/i.test(h);
}

function resolveBlurTarget(blockElement, result) {
  const assistantRoot = findAssistantMessageRoot(blockElement);
  if (assistantRoot) {
    const len = (assistantRoot.textContent || '').length;
    if (len >= 20 && len < 15000 && !assistantRoot.getAttribute('data-spoiler-shield')) {
      // AI chat: blur paragraph only unless critical spoiler
      if (isAiChatSite() && result?.severity !== 'critical') {
        return blockElement;
      }
...
function processTextWithV4(node, trackedMedia, store, processedBlocks, analysisCache) {
  ...
  try {
    ...
    const blurTarget = resolveBlurTarget(blockElement, result);
